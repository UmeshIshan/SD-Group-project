...
class CustomConfig(Config):
    ...
    NUM_CLASSES = 1 + 2  # Background + Dental_caries, periodontal
...
inference_config = InferenceConfig()                     ##storing inferenceconfig in a variable       
...
def predict():
    # Load the image
    image_data = request.files["image"].read()                                                      ##reading the imput image  

    image = cv2.imdecode(np.frombuffer(image_data, np.uint8), cv2.IMREAD_COLOR)                     ##reading the file using open cv
    ...
    # Run inference
    results = model.detect([image], verbose=1)                                                      ##peforming the detection
    ...
    class_names = ['BG', 'Dental_Caries', 'Periodontal']
